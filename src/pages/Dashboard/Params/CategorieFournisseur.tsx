...
const {mutate:createClasse,isLoading:loadingCreate} = useMutation((data) => categoriesFournisseurService.create(data),{
    onSuccess:(_) => {
        message.success("La categorie a été ajouté avec succès");
        close();
        qc.invalidateQueries(key);
        form.reset();
    },
    onError:(_) => {
    message.error("Impossible d'ajouter le categorie");
    }
});

const {mutate:updateClasse,isLoading:loadingU} = useMutation((data: any) => categoriesFournisseurService.update(data.id, data.data),{
    onSuccess:(_) => {
        message.success("La categorie a été ajouté avec succès");
        closeU();
        qc.invalidateQueries(key);
    ...
    onError:(_) => {
    message.error("Impossible d'ajouter la categorie");
    }
});

const {mutate:deleteClasse,isLoading:loadingDelete} = useMutation((id:string) => categoriesFournisseurService.delete(id),{
    onSuccess:(_) => {
...
const handleUpdate  = (data: any) => {
  ...
  return (
    ...
    <LoadingOverlay
       ...
       />
    <div className="my5 p-2 text-2xl font-semibold flex">GESTION  DES CATEGORIES</div>
   ...
   <DataTable value={data} paginator rows={10} size="small" stripedRows filters={filters} rowsPerPageOptions={[5, 10, 25, 50]} loading={isLoading} tableStyle={{ minWidth: '50rem' }}
    globalFilterFields={['libelle']} header={header}>
   <Column field="libelle" header="LIBELLE" style={{ width: '25%' }}></Column>
   <Column headerStyle={{ width: '4rem', textAlign: 'center' }} bodyStyle={{ textAlign: 'center', overflow: 'visible' }} body={actionBodyTemplate} />
</DataTable>
   <Drawer opened={opened} onClose={close} title="CREATION  DE CATEGORIE">
   <LoadingOverlay
   ...
   <Drawer position="right" opened={openedU} onClose={closeU} title="MODIFICATION  DE CATEGORIE">
   <LoadingOverlay
         visible={loadingU}
         zIndex={1000}
         overlayProps={{ radius: 'sm', blur: 2 }}
...
export default CategorieFournisseur